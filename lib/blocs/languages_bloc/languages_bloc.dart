...
class LanguagesBloc extends Bloc<LanguagesEvent, LanguagesState> {
  ...
  Future<void> _onLoadLanguages(
      LoadLanguages event, Emitter<LanguagesState> emit) async {
  ...
  Future<void> _onAddLanguages(
      AddLanguages event, Emitter<LanguagesState> emit) async {
    try {
      await userService.addLanguages(event.languages);
      final updateLanguages = List<Map<String, dynamic>>.from(state.languages)
        ..add(event.languages);
  ...
  Future<void> _onRemoveLanguages(
      RemoveLanguages event, Emitter<LanguagesState> emit) async {
    try {
      await userService.removeLanguages(event.languages);
      final updateLanguages = List<Map<String, dynamic>>.from(state.languages)
        ..remove(event.languages);
      emit(state.copyWith(languages: updateLanguages));
    } catch (e) {
      emit(state.copyWith(error: e.toString()));
    }
