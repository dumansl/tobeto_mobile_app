...
class ProjectsPrizeBloc extends Bloc<ProjectsPrizeEvent, ProjectsPrizeState> {
  ...
  ProjectsPrizeBloc({required this.userService}) : super(ProjectsPrizeState()) {
  ...
  Future<void> _onLoadProjectsPrize(LoadProjectsPrize event, Emitter<ProjectsPrizeState> emit) async {
  ...
  Future<void> _onAddProjectsPrize(AddProjectsPrize event, Emitter<ProjectsPrizeState> emit) async {
    try {
      await userService.addProjectsPrize(event.projects);
      final updateProjects = List<Map<String, dynamic>>.from(state.projects)..add(event.projects);
  ...
  Future<void> _onRemoveProjectsPrize(RemoveProjectsPrize event, Emitter<ProjectsPrizeState> emit) async {
    try {
      await userService.removeProjectsPrize(event.projects);
      final updateProjects = List<Map<String, dynamic>>.from(state.projects)..remove(event.projects);
      emit(state.copyWith(projects: updateProjects));
    } catch (e) {
      emit(state.copyWith(error: e.toString()));
    }
