...
class CertificateBloc extends Bloc<CertificateEvent, CertificateState> {
  ...
  CertificateBloc() : super(CertificateState()) {
  ...
  Future<void> _onLoadCertificate(
      LoadCertificate event, Emitter<CertificateState> emit) async {
  ...
  Future<void> _onAddCertificate(
      AddCertificate event, Emitter<CertificateState> emit) async {
    try {
      await userService.addCertificate(event.certificate);
      final updateCertificate =
          List<Map<String, dynamic>>.from(state.certificate)
            ..add(event.certificate);
  ...
  Future<void> _onRemoveCertificate(
      RemoveCertificate event, Emitter<CertificateState> emit) async {
    try {
      await userService.removeCertificate(event.certificate);
      final updateCertificate =
          List<Map<String, dynamic>>.from(state.certificate)
            ..remove(event.certificate);
      emit(state.copyWith(certificate: updateCertificate));
    } catch (e) {
      emit(state.copyWith(error: e.toString()));
    }
